...
% Diagrama de balanceo de carga con TikZ (corregido para modo LR)
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{capturas/diagrama-balanceo.png}
  \caption{Diagrama de arquitectura: balanceo de carga con Nginx entre dos servidores backend.}
  \label{fig:tikz-balanceo}
\end{figure}
...
\section{Implementación alternativa con Docker}
...
\subsection{Arquitectura con contenedores Docker}
...
\subsection{Configuración de la red Docker}
...
\subsection{Despliegue de servidores backend}
...
Cada contenedor expone su puerto 80 interno a puertos diferentes del host (8081 y 8082) para permitir acceso directo durante las pruebas. Se utiliza la imagen \texttt{nginx:alpine} por su tamaño reducido y eficiencia.

\subsection{Personalización de páginas backend}
...
\subsection{Configuración del balanceador de carga}

Se crea un archivo de configuración para el balanceador Nginx (\texttt{nginx-lb.conf}):
...
Nótese que se utilizan los nombres de los contenedores (\texttt{backend1} y \texttt{backend2}) en lugar de direcciones IP, gracias a la resolución DNS proporcionada por la red Docker.

\subsection{Despliegue del balanceador}
...
\begin{enumerate}
    \item \textbf{Verificar estado de contenedores:}
    \begin{verbatim}
    docker ps
    \end{verbatim}
    
    \item \textbf{Probar acceso directo a backends:}
    \begin{verbatim}
    curl http://localhost:8081  # Backend 1
    curl http://localhost:8082  # Backend 2
    \end{verbatim}
    
    \item \textbf{Probar balanceador de carga:}
    \begin{verbatim}
    for i in {1..10}; do curl http://localhost; done
    \end{verbatim}
    Se observará la alternancia entre ``Backend Server 1'' y ``Backend Server 2''.
    
    \item \textbf{Verificar logs del balanceador:}
    \begin{verbatim}
    docker logs load-balancer
    \end{verbatim}
    
    \item \textbf{Prueba de alta disponibilidad:}
    \begin{verbatim}
    docker stop backend1
    curl http://localhost
    \end{verbatim}
    Todas las peticiones serán atendidas por backend2. Al reiniciar backend1 con \texttt{docker start backend1}, el balanceo se reanuda automáticamente.
\end{enumerate}

\subsection{Docker Compose para orquestación}

Para simplificar el despliegue, se puede utilizar Docker Compose con un archivo \texttt{docker-compose.yml}:
...
Con este archivo, todo el stack se despliega con un solo comando: \texttt{docker-compose up -d}

\subsection{Ventajas de la implementación con Docker}

La implementación con contenedores Docker ofrece múltiples ventajas sobre la virtualización tradicional:

\begin{itemize}
    \item \textbf{Menor consumo de recursos:} Los contenedores comparten el kernel del host, utilizando menos memoria y CPU que máquinas virtuales completas.
    \item \textbf{Inicio rápido:} Los contenedores se inician en segundos, mientras que las VMs requieren minutos.
    \item \textbf{Portabilidad:} La configuración se puede replicar fácilmente en diferentes entornos mediante archivos de configuración.
    \item \textbf{Escalabilidad:} Es trivial agregar más contenedores backend con \texttt{docker-compose scale}.
    \item \textbf{Aislamiento:} Cada contenedor opera de forma aislada, mejorando la seguridad.
    \item \textbf{Versionado:} Las imágenes Docker permiten mantener versiones específicas de software.
    \item \textbf{Integración CI/CD:} Docker se integra perfectamente con pipelines de integración y despliegue continuo.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{capturas/Docker/containers.png}
    \caption{Vista de contenedores Docker ejecutando la arquitectura de balanceo de carga en OrbStack.}
